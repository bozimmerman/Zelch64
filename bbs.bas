!--------------------------------------------------
!- Saturday, May 20, 2017 3:37:29 AM
!- Import of : 
...
3 GET#5,C$:IFC$<>"0"ANDC$<>"o"THEN3
4 RETURN
10 PRINTCHR$(14)
11 GOTO25
12 IFP0>200ORE$=""THENRETURN
13 P0$(P0)=E$:P0=P0+1:RETURN
25 CLOSE5:OPEN5,2,3,CHR$(6)+CHR$(0):GET#5,CC$:A$="v0x1":GOSUB2
30 DIMTT$(50),TT(50),Z1(50),Z2(50),P0$(200),DA(12):POKE53280,0
...
110 RETURN
111 I=0:SYS49158:IFPEEK(830)>0THEN1230
...
200 RETURN
210 IFTX$=""THENRETURN
220 SYS 49161,TX$:POKE710,0:RETURN
...
320 NEXTI:INPUT#8,UL,CH$,CI$:CLOSE8:CLOSE1
...
430 PRINT"{light blue}Last caller=";L1$
...
680 GOTO630
690 CLOSE1:CLOSE2:CLOSE7:CLOSE8:FORI=0TO500:NEXT:GET#5,A$:GET#5,A$
...
1040 TX$="{yellow}Welcome Back "+NA$+"!":GOSUB210:POKE733,PEEK(733)+1:GOTO1060
...
1070 TX$="{white}You may make "+STR$((CA(AC))-VAL(CA$))+" more calls today."
...
1320 OPEN1,8,15,"s0:{pound}last caller":CLOSE1:SYS49167
1330 OPEN8,8,8,"{pound}last caller,s,w":PRINT#8,NA$+"  ":LC$=LC$+" "+I$:PRINT#8,LC$
1340 PRINT#8,LZ+1:PRINT#8,HN:CLOSE8:OPEN1,8,15,"i":CLOSE1:POKE702,16
1345 I=300:FH=1:GOSUB6230
1346 PRINT#5,CHR$(13):GOSUB85:GOSUB85:A$="z":GOSUB2:GOSUB80
1347 CLOSE5:RUN
...
1850 IFPEEK(681)=1THENTX$="{white}Change to {reverse on}Standard Ascii{reverse off} for the":GOSUB210
1851 IFPEEK(681)=1THENTX$="NEW USER process then hit {reverse on}RETURN{reverse off}":GOSUB210
1852 IFPEEK(681)=1THENGOSUB100:IFIN$<>CHR$(13)THEN1852
1853 POKE681,0
1855 T1=1:T2=0:GOSUB1700:TX$=CHR$(13)+"Finding an opening. Standby...{ct c}":GOSUB210
1860 C1=1:C2=0:OPEN1,8,15:OPEN2,8,2,"{pound}userlog"
...
1940 TX$="Enter your HANDLE (17 chars or less)"+Q$+":{ct c}":GOSUB210
1950 GOSUB190:NN$=I$:IFLEN(I$)>17ORI$=""THEN1940
1960 TX$="Enter your PHONE{sh space}NUMBER (XXX-XXXX)"+Q$+":{ct c}":GOSUB210
1970 GOSUB190:PH$=I$:IFLEN(I$)>9ORI$=""THEN1960
1980 TX$="Enter your NAME (17 chars or less)"+Q$+":{ct c}":GOSUB210
1990 GOSUB190:RN$=I$:IFLEN(I$)>17ORI$=""THEN1980
2000 TX$="Enter a PASSWORD (7 chars or less)"+Q$+":{ct c}":GOSUB210
2010 Q$=CHR$(13):GOSUB190:PS$=I$:IFLEN(I$)>9ORI$=""THEN2000
2020 TX$=Q$+Q$+"Handle:"+NN$:GOSUB210:TX$="Phone:"+PH$:GOSUB210:TX$="Name:"+RN$
2030 GOSUB210:TX$="Password:"+PS$:GOSUB210:TX$=Q$+"Is this correct?{ct c}":GOSUB210
...
2110 TX$=" "+Q$:GOSUB210:FI$="{pound}newuser":GOSUB1200
2115 GOSUB6700:GOTO750
...
2345 OPEN8,8,8,"{pound}members,s,r":S$=I$:I=0:SYS49182:TX$=I$:GOSUB210
2350 SYS49182:IFI$=""THENCLOSE8:GOTO2120
...
3646 IFI$<>""THENTT$(6)=I$
3647 FORI=7TO12:TX$="Var"+STR$(I-7)+":"+TT$(I)+":{ct c}":GOSUB210:GOSUB190
...
3735 TX$="{clear}{down*2}{light blue} Email Section...":GOSUB210:OPEN8,MD,8,"{pound}mail"+STR$(NA)
...
4635 TX$="y":GOSUB210
4640 QQ$="REPLY BY:"+NA$+"{reverse off}{white} ID#"+STR$(NA):SYS49167:QD$="DATE:"+STR$(MN)+"/"
...
4890 SYS49167:QD$="TIME:"+I$+" SUBJECT:"+S$+"{reverse off}{white}"+Q$:IFSF(CS)>NMTHENNM=NM+1
...
5030 IFI$<>"y"THEN5010
5040 PRINT#9,CHR$(1)
5045 SYS49182:IFI$=CHR$(1)THEN5070
...
5166 GOSUB210:OPEN8,UV(CU),0,"$:z=u":SYS49197:CLOSE8:TX$="{white}{down}There are"
...
5620 CLOSE1:CLOSE2:PRINT#5,"loading system ml":I$="{pound}v2.3 ml1":SYS51971,I$
...
5730 GOTO5700
5740 TX$="{clear}{down}{pink}Loading Vote!":GOSUB210:OPEN1,8,15:OPEN8,8,8,"{pound}vote,s,r":INPUT#1,E1
5750 I1=0:I2=0:I1$="":IFE1>0THENCLOSE1:CLOSE8:GOTO2120
5760 INPUT#8,I1$:INPUT#8,I2:FORI=0TOI2:INPUT#8,I1$(I):INPUT#8,I2$(I):I1(I)=0
5770 NEXTI:CLOSE8:TX$="{down*2}{yellow}Loading Results!":GOSUB210:OPEN8,8,8,"{pound}vote box,s,r"
...
5930 TX$="{light blue}{clear}{down*2}Results!":GOSUB210:O=0:FORI=0TOI2:O=O+I1(I):NEXT
5940 TX$="{light green}{down}Out of"+STR$(O)+" votes...":GOSUB210:FORI=0TOI2
5945 TX$=STR$(I1(I))+"-"+I2$(I)+" ("+STR$(INT((I1(I)/O)*100))+"% )":GOSUB210
5950 NEXTI:TX$="{down}Hit RETURN{ct c}":GOSUB210
...
6060 TX$="{light green}(Q/S)/Option:{ct c}":GOSUB210
...
6500 TX$=CHR$(13)+"{pink}H)i Msg:"+STR$(PP):GOSUB210:TX$="{yellow}P)assword:{ct c}":GOSUB210
6510 TX$=PS$:POKE710,1:GOSUB210:TX$="{white}B)aud:"+P$(2):GOSUB210:TX$="{light green}Choice:{ct c}"
6511 GOSUB210:GOSUB190:IFI$="h"THEN6520
6512 IFI$="p"THEN6540
6513 IFI$="b"THEN6560
6514 IFI$=""THEN2120
6515 GOTO6500
...
6541 TX$="{yellow}Verify:{white}{ct c}":GOSUB210:POKE710,1:GOSUB190:IFI$<>O$THEN6599
6542 PS$=I$:GOTO6500
...
6820 PRINT"{yellow}term active":SYS49188
...
6910 OPEN1,8,15:OPEN2,8,2,"{pound}userlog":OPEN8,8,8,"{pound}members,s,w":I1=1:I2=0
...
6950 SYS49185:I$=A$+I$:IFI$="EMPTY"THEN6970
...
7010 FH=1:I$="{pound}p.protocol":SYS51971,I$:OPEN1,8,15:OPEN2,8,2,A$+","+B$+",r"
...
7110 FH=1:I$="{pound}p.protocol":SYS51971,I$:FI$=A$+","+B$+",w":GOTO5420
