...
151 POKE56334,PEEK(56334)AND254:POKE1,PEEK(1)AND251:POKE789,192:POKE788,15
152 POKE1,PEEK(1)OR4:POKE56334,PEEK(56334)OR1
...
310 RETURN
320 TX$="Call back sometime after midnight!":GOSUB530:GOTO2060
...
860 INPUT#1,E1:INPUT#8,MD:INPUT#8,MO:DIMM1$(MO),M2(MO),M3$(MO)
861 FORI=0TOMO:INPUT#8,M1$(I):INPUT#8,M2(I)
870 INPUT#8,M3$(I):NEXT
890 CLOSE8:CLOSE1
900 OPEN8,8,8,"{pound}last caller,s,r":INPUT#8,L1$:INPUT#8,L2$:INPUT#8,LZ
910 CLOSE8:CLOSE1
...
1000 H=H1*10+H2:IFCH=128THENH=H+12
1010 IFH=23ANDPEEK(712)<>15THENPOKE712,15:GOTO3280
1020 IFPEEK(712)=15ANDH<13THENPOKE712,0
...
1310 TX$=CHR$(13)+"{yellow}Enter your ID number      :":GOSUB530:GOSUB470
...
1380 IFI$=PS$THENTX$="{clear}{light green}Loading info":GOSUB530:GOSUB3020:GOTO1420
...
1420 GOSUB3700:IFPEEK(709)=1THENOPEN4,4,7:PRINT#4,NA$;":";TT$:PRINT#4:CLOSE4
1430 TX$="{light blue}Welcome Back "+NA$+"!":GOSUB530:GOTO1450
1440 GOSUB3020
1450 AC=VAL(AC$):CA=VAL(CA$):IFCA>CA(AC)THENTX$="{red}Daily Call Number Exceeded!"
1460 IFCA>CA(AC)THENGOSUB530:GOTO2060
1470 C4$=STR$(CA(AC)-CA):TX$="{cyan}You may make "+C4$+" more calls today.":GOSUB530
1480 CLOSE2:CLOSE1:GOSUB3580
...
1530 TX$="{white}You Have Mail!":GOSUB530:GOTO1630
1630 OPEN8,8,8,"{pound}newsfile,s,r":GET#8,A$:CLOSE8
...
2060 IFPEEK(830)=1THENTX$="{red}Time UP!!":GOSUB530
...
2110 TM$=TT$:LC$=STR$(MN)+"/"+STR$(DA):CA$=STR$(CA+1):C2=INT(NA/256):C3=C1*256
...
2695 PRINT#1,"p"CHR$(98)CHR$(C1)CHR$(C2)CHR$(0)
2699 GOTO2710
...
3280 PRINT"{clear}{down*3}{reverse on}{green}monster mode"
...
3330 PRINT"{home}{down*5}";STR$(C1+(256*C2))+"{left}   "
...
3630 M1=M1+TW:IFM1<6THEN3670:
3640 M1=M1-5
3650 H2=H2+1:IFH2>=10ANDH1=0THENH2=H2-10:H1=1:GOTO3680
3660 IFH2>2ANDH1=1THENH2=1:H1=0:CH=128
3670 IFH1=1ANDH2=1ANDCH=128ANDM1>3ANDPEEK(712)=0THEN320
...
3721 H2$=RIGHT$(STR$(H2),1):M1$=RIGHT$(STR$(M1),1):M2$=RIGHT$(STR$(M2),1)
3722 H1$="":IFH1>0THENH1$="1"
3730 TT$=H1$+H2$+"]"+M1$+M2$+TT$:RETURN
