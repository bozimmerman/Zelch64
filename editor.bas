...
340 END
350 REM
360 IFFL=1THENFL=0:OPEN1,8,15:OPEN2,8,2,"{pound}userlog":GOTO1000
370 PRINT"{clear}1) Create new user log"
380 PRINT"2) Edit User Log"
...
460 IFA$<>"2"THEN420
470 GOTO600
480 INPUT"start at record";S1
490 PRINT"space aborts..."
500 OPEN1,8,15:OPEN2,8,2,"{pound}userlog"
510 I1=INT(S1/256):I3=I1*256:I2=S1-I3:
520 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(5):GET#2,A$:INPUT#1,E1
530 IFE1<>0ORA$=""THEN580
540 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(5)
550 INPUT#2,NA$:PRINT"#";S1;" ";NA$:GETA$:IFA$=" "THEN580
560 S1=S1+1:GOTO510
570 GOTO510
580 CLOSE2:CLOSE1:PRINT"hit any key"
590 POKE198,0:WAIT197,64,64:GOTO360
600 INPUT"enter the record to edit";S1:I1=INT(S1/256):I3=I1*256:I2=S1-I3
610 OPEN1,8,15:OPEN2,8,2,"{pound}userlog"
620 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(5):GET#2,A$
630 IFST=0ANDA$<>""THENGOTO670
640 PRINT"no record":CLOSE1:CLOSE2:FORI=0TO1000:NEXT:GOTO360
650 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(I):INPUT#2,A$
660 RETURN
670 I=5:GOSUB650:NA$=A$:I=25:GOSUB650:PH$=A$:I=35:GOSUB650:AC$=A$:
680 I=40:GOSUB650:RN$=A$:I=60:GOSUB650:LA$=A$
690 I=80:GOSUB650:TM$=A$:I=90:GOSUB650:PA$=A$:I=100:GOSUB650
700 O=0:FORI=100TO145STEP5:GOSUB650:P$(O)=A$:O=O+1:NEXT:CLOSE2:CLOSE1
710 PRINT"name:";NA$:PRINT"Phone:";PH$:PRINT"Access lvl:";AC$:
720 PRINT"Real Name:";RN$:PRINT"Last Call Date:";LA$
730 PRINT"off at:";TM$:PRINT"password:";PA$:FORI=0TO9
740 PRINT"variable #"I" = ";P$(I):NEXT:PRINT"Edit this record?"
750 GETA$:IFA$="n"THENCLOSE2:CLOSE1:GOTO360
760 IFA$<>"y"THEN750
770 INPUT"name:";NA$:INPUT"Phone:";PH$:INPUT"Access lvl:";AC$:
780 INPUT"Real Name:";RN$:INPUT"Last Call Date:";LA$:INPUT"Calls today:";CA$
790 INPUT"off at:";TM$:INPUT"password:";PA$:FORI=0TO9
800 PRINT"variable #"I" = ";:INPUTP$(I):NEXT:PRINT"Save this record?"
810 GETA$:IFA$="n"THEN710
820 IFA$<>"y"THEN810
830 OPEN1,8,15:OPEN2,8,2,"{pound}userlog":GOTO1000
840 PRINT"Delete old userlog and make new one"
850 PRINT"ok? (y/n)"
860 GETA$:IFA$="n"THEN360
870 IFA$<>"y"THEN860
880 PRINT"enter your name (20 chars)":INPUTNA$
890 PRINT"enter the phone #(10 chars)":INPUTPH$
900 AC$="9":INPUT"your real name(20 chars)";RN$
910 LA$="1/ 1":TM$="12:59":INPUT"password (7 chars)";PA$
920 FORI=0TO9:P$(I)="0":NEXT
930 PRINT"take this questionare again (Y/N)"
940 GETA$:IFA$="y"THEN880
950 IFA$<>"n"THEN940
960 OPEN1,8,15,"s0:{pound}userlog":I2=1:I1=0
970 OPEN2,8,2,"{pound}userlog,l,"+CHR$(150)
980 GOTO1000
990 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(I):RETURN
1000 I=5:GOSUB990:PRINT#2,NA$:I=25:GOSUB990:PRINT#2,PH$:I=35:GOSUB990:
1010 PRINT#2,AC$:I=40:GOSUB990:PRINT#2,RN$:I=60:GOSUB990:PRINT#2,LA$
1020 I=80:GOSUB990:PRINT#2,TM$:I=90:GOSUB990:PRINT#2,PA$:CT=0
1030 FORI=100TO145STEP5:GOSUB990:PRINT#2,P$(CT):CT=CT+1:NEXTI
1040 CLOSE2:CLOSE1
1050 PRINT"Hit any key"
1060 POKE198,0:WAIT197,64,64:GOTO360
1070 GOTO130
1080 PRINT"Finding a Record...":OPEN1,8,15:OPEN2,8,2,"{pound}userlog":I2=1:I1=0
1090 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(5):INPUT#1,E:GET#2,A$
1100 IFE>5ORA$=""THEN1140
1110 PRINT#1,"p"CHR$(98)CHR$(I2)CHR$(I1)CHR$(5):INPUT#2,A$:IFA$="EMPTY"THEN1140
1120 I2=I2+1:IFI2=256THENI2=0:I1=I1+1
1130 GOTO1090
1140 CLOSE1:CLOSE2:INPUT"Handle";NA$:IFLEN(NA$)>17ORNA$=""THEN1140
1150 INPUT"Real name";RN$:IFLEN(RN$)>17ORRN$=""THEN1150
1160 INPUT"Phone";PH$:IFLEN(PH$)>8ORPH$=""THEN1160
1170 INPUT"Access";AC$:IFLEN(AC$)>2ORAC$=""THEN1170
1180 CA$="0":LA$="1/1":TM$="12:00":FORI=0TO9:P$(I)="0":NEXT
1190 INPUT"Password";PA$:IFLEN(PA$)>7ORPA$=""THEN1190
1200 PRINT"S)ave or Q)uit"
1210 GETA$:IFA$="q"THEN360
1220 IFA$="s"THENFL=1:OPEN1,8,15:OPEN2,8,2,"{pound}userlog":GOTO1000
1230 GOTO1210
1240 REM
