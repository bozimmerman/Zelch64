!--------------------------------------------------
!- Saturday, May 20, 2017 4:12:10 PM
!- Import of : 
...
8030 TX$="{f1}Enter your PHONE{sh space}NUMBER (XXX-XXXX)"+Q$+":{ct c}":GOSUB210
8040 GOSUB190:PH$=I$:IFLEN(I$)>9ORI$=""THEN8030
...
8236 C1=1:C2=0:OPEN1,8,15,"i0":OPEN2,8,3,"0:{pound}userlog"
8237 I=5:GOSUB6:GET#2,A$:INPUT#1,E1
8238 IFE1>0ORA$=""THEN8245
8239 SYS38233
8240 I$=A$+I$:IFI$="EMPTY"THEN8245
...
8270 P$(I)="0":NEXTI:TX$=NA$:GOSUB210:TX$=Q$+Q$+"{f1}Your ID is:"+STR$(ID)
8280 GOSUB210:TX$="{f1}Your PASSWORD is:"+PS$:GOSUB210:NA=ID
8285 TX$="{f1}Saving info...":GOSUB210:NA$=NN$
8287 OPEN1,8,15,"i0":OPEN2,8,3,"{pound}userlog":C2=INT(ID/256):C3=C2*256:C1=ID-C3
8288 I=145:GOSUB6:PRINT#2,"0":SYSZ,2:SYSZ,1
8290 Y0=1:GOSUB1350:NA$="":TX$="{f1}Press RETURN{ct c}":GOSUB210
